...
for genome_folder in glob.glob("EP_results_bact/*"):
    genome_id = genome_folder.replace("EP_results_bact/", "")

    if not os.path.exists(f"EP_results_bact/{genome_id}/{genome_id}/pipolins"):
        print(f"There is not pipolins folder for {genome_id}")
        continue

    for pipolin in os.listdir(f"EP_results_bact/{genome_id}/{genome_id}/pipolins"):
        if "single_record" not in pipolin and "v0" in pipolin and ".gbk" in pipolin:
            ...
            for record in SeqIO.parse(f"EP_results_bact/{genome_id}/{genome_id}/pipolins/{pipolin}", "genbank"):
                pipolin_fragments += 1
            ...
            for record_rec in SeqIO.parse(f"EP_results_bact/{genome_id}/{genome_id}/pipolins/{pipolin.replace(".gbk", ".single_record.gbk")}", "genbank"):
            ...
            pipolin_contig_ids = [re.id for re in SeqIO.parse(f"EP_results_bact/{genome_id}/{genome_id}/pipolins/{pipolin_fa}", "fasta")]
            ...
            for fa in SeqIO.parse(f"EP_results_bact/{genome_id}/{genome_id}/pipolins/{pipolin_fa}", "fasta"):
            ...
            for record in SeqIO.parse(f"EP_results_bact/{genome_id}/{genome_id}/pipolins/{pipolin.replace(".gbk", ".single_record.gbk")}", "genbank"):
                for pipolin_feat in record.features:
                    if "assembly_gap" in str(pipolin_feat.type):
...
#add haeder
table_output = "Pipolin_ID\tassembly\tGenome_ID\tPipolin_file\tPipolin_scaffold\tpiPolB_num\tpiPolB_fragments_length\tpiPolB_fragments_length_sum\t"
...
with open("info_from_EP_v2.txt", "w") as f:
